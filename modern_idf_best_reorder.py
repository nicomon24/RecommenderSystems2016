...
OUTPUT_FILE = str(time.time()).split('.')[0] + "RETRY" + '.csv'
#--------- TUNING --------
#Dataset tuning
LAST_INTERACTION_DAYS = 24
TRENDING_DAYS = 4
#Confidence
...
TOP_CONTENT_LENGHT = 11
TOP_COLLAB_LENGHT = 11
...
items_last_4_days = defaultdict(int)
items_last_nico = defaultdict(float)
for i in interactions_active:
    if i[3] > max_timestamp - (24*60*60 * LAST_INTERACTION_DAYS):
        items_last_week[i[1]] = i[3]
    if i[3] > max_timestamp - (24*60*60 * TRENDING_DAYS):
        items_last_4_days[i[1]] += 0.7 - 0.3 * float(i[3] - max_timestamp) / (24*60*60 * TRENDING_DAYS)
        items_last_nico[i[1]] += 0.2 + 0.8 * (TRENDING_DAYS - (i[3] - max_timestamp)/(24*60*60))
...
# ----- TIMESTAMPS -----
timestamp_mean = defaultdict(float)
timestamp_var = defaultdict(float)
timestamp_count = defaultdict(int)
max_timestamp = 0
for interaction in interactions:
    #Translate TS in days
    ts = interaction[3] / (24 * 60 * 60)
    if ts > max_timestamp:
        max_timestamp = ts
    item = interaction[1]
    timestamp_mean[item] += ts
    timestamp_count[item] += 1
    timestamp_var[item] += ts**2
# Finished counting
for item in timestamp_count:
    timestamp_mean[item] = timestamp_mean[item] / float(timestamp_count[item])
    timestamp_var[item] = math.sqrt((timestamp_var[item] / float(timestamp_count[item])) - timestamp_mean[item]**2)

print "recommendable_items before 2.0: " + str(len(recommendable_items))

threshold = max_timestamp - 12
for item in timestamp_mean:
    max_ts = timestamp_mean[item] + timestamp_var[item] / 2
    if max_ts < threshold:
        recommendable_items.discard(item)

print "recommendable_items final: " + str(len(recommendable_items))
...
def content(target, likes):
    ...
    j = 0
    while len(top_interests) < 8:
        if toppop[j] not in likes:
            top_interests.append(toppop[j])
        j += 1
    return top_interests[:TOP_CONTENT_LENGHT]
...
def collaborative(user_id):
    ...
    for i in likes_keys:
        for j in similar_items:
            ...
            sim += tagsTitlesSimilarity(i,j) * TAGS_TITLES_BOOST
    ...
    # If user is not of a high career_level, slow down higher levels (more than 3)
    '''
    if users[user_id][2] < 4:
        for item in scores:
            if items[item][2] >= 4:
                scores[item] /= 1.2
    '''
    # if something goes wrong put toppop
    top = sorted(scores, key=scores.get,reverse = True)[:TOP_COLLAB_LENGHT]
    for i in top:
...
def resort(reco):
    ...
    for k in range(0,len(reco)):
        item = reco[k]
        reco_dict[item] += 1.5 * (len(reco) - k) + items_last_4_days[item]
        #print item,reco_dict[item]
        if items[item][11] > 1447023571 - 20*24*60*60:
            reco_dict[item] += 2
        if items[item][11] > 1447023571 - 30*24*60*60:
            reco_dict[item] += 1
    resorted = sorted(reco_dict, key=reco_dict.get,reverse = True)
    return resorted[:5]
...
for target in targets:
    ...
    if len(top) < 5:
        top.extend(toppop)
    top = resort(top)[:5]
    fout.write(str(target) + "," + " ".join(map(str, top)) + "\n")
    print top
fout.close()
